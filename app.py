...
MODEL_DIR = "model"
MODEL_PATH = os.path.join(MODEL_DIR, "f1_pipeline.pkl")
...
year = st.number_input("Year (use latest or override)", min_value=1950, max_value=2100, value=mappings.get("default_year", 2023), step=1)

st.markdown("---")
st.write("Model test performance (on held-out year):")
st.write(f"Accuracy: **{mappings['metrics']['accuracy']:.3f}**, F1: **{mappings['metrics']['f1']:.3f}**, test year: **{mappings['metrics']['test_year']}**")

if st.button("Predict podium probability"):
    # map human names -> ids (keys are lowercase in mappings)
    ...
    if driver_id is None or ctor_id is None or circuit_id is None:
        ...
        st.progress(proba)  # nice visual indicator
